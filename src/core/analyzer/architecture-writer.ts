...
/** Locale-aware role badge label. Computed per-call so it respects the active locale. */
function roleBadge(role: string): string {
  const key: Record<string, Parameters<typeof t>[0]> = {
    entry_layer:    'arch.roleEntryLayer',
    orchestrator:   'arch.roleOrchestrator',
    core_utilities: 'arch.roleCoreUtilities',
    api_layer:      'arch.roleApiLayer',
    internal:       'arch.roleInternal',
  };
...
export function renderArchitectureMarkdown(overview: ArchitectureOverview): string {
  ...
  lines.push(t('arch.overviewTitle'));
  lines.push('');
  lines.push(t('arch.generatedBy', { date: overview.generatedAt.slice(0, 10) }));
  lines.push(t('arch.rerunNote'));
  lines.push('');

  // Summary table
  lines.push(t('arch.summary'));
  lines.push('');
  lines.push(t('arch.summaryTableHeader'));
  lines.push(t('arch.summaryTableSep'));
  lines.push(t('arch.metricFiles', { value: summary.totalFiles }));
  lines.push(t('arch.metricModuleClusters', { value: summary.totalClusters }));
  lines.push(t('arch.metricDependencyEdges', { value: summary.totalEdges }));
  if (summary.cycles > 0) {
    lines.push(t('arch.metricCycles', { value: summary.cycles }));
  }
  if (summary.layerViolations > 0) {
    lines.push(t('arch.metricLayerViolations', { value: summary.layerViolations }));
  }
  lines.push('');

  // Cluster map
  lines.push(t('arch.moduleClusters'));
  lines.push('');
  lines.push(t('arch.moduleClustersProse'));
  lines.push('');
  lines.push(t('arch.clustersTableHeader'));
  lines.push(t('arch.clustersTableSep'));
  for (const cl of clusters) {
    const role = roleBadge(cl.role);
  ...
  if (withKeys.length > 0) {
    lines.push(t('arch.keyFilesPerCluster'));
    lines.push('');
    for (const cl of withKeys) {
      lines.push(`**${cl.name}** (\`${roleBadge(cl.role)}\`)`);
  ...
  if (globalEntryPoints.length > 0) {
    lines.push(t('arch.entryPoints'));
    lines.push('');
    lines.push(t('arch.entryPointsProse'));
    lines.push('');
  ...
  if (criticalHubs.length > 0) {
    lines.push(t('arch.criticalHubs'));
    lines.push('');
    lines.push(t('arch.criticalHubsProse'));
    lines.push('');
    lines.push(t('arch.hubsTableHeader'));
    lines.push(t('arch.hubsTableSep'));
