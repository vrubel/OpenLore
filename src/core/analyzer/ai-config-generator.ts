...
function buildContent(analysisDir: string, projectName: string, forClaude: boolean): string {
  // The reference below appends a literal `/`, so the directory has to speak the same
  // dialect. A caller that passes a host-OS path (`.openlore\analysis` on Windows)
  // would otherwise mint `@.openlore\analysis/CODEBASE.md` — one reference in two
  // dialects, written into a file that is created once and then committed. Normalising
  // here as well as at the call site keeps any future caller from re-introducing it.
  const dir = analysisDir.replace(/\\/g, '/').replace(/\/$/, '');
  const digestRef = forClaude
    ? `@${dir}/CODEBASE.md`
    : `<!-- Import or paste ${dir}/CODEBASE.md here for full project context -->`;
